...
class JiebaTestCase(unittest.TestCase):
    ...
    def testDefaultCut(self):
        for content in test_contents:
            ...
            assert isinstance(
                result, types.GeneratorType), "Test DefaultCut Generator error"
            result = list(result)
            assert isinstance(
                result, list), "Test DefaultCut error on content: %s" % content
    ...
    def testCutAll(self):
        for content in test_contents:
            ...
            assert isinstance(
                result, types.GeneratorType), "Test CutAll Generator error"
            result = list(result)
            assert isinstance(
                result, list), "Test CutAll error on content: %s" % content
            print(" , ".join(result), file=sys.stderr)
    ...
    def testSetDictionary(self):
        ...
        for content in test_contents:
            result = jieba.cut(content)
            assert isinstance(
                result, types.GeneratorType), "Test SetDictionary Generator error"
            result = list(result)
            assert isinstance(
                result, list), "Test SetDictionary error on content: %s" % content
            print(" , ".join(result), file=sys.stderr)
    ...
    def testCutForSearch(self):
        for content in test_contents:
            ...
            assert isinstance(
                result, types.GeneratorType), "Test CutForSearch Generator error"
            result = list(result)
            assert isinstance(
                result, list), "Test CutForSearch error on content: %s" % content
            print(" , ".join(result), file=sys.stderr)
    ...
    def testPosseg(self):
        ...
        for content in test_contents:
            ...
            assert isinstance(
                result, types.GeneratorType), "Test Posseg Generator error"
            result = list(result)
            assert isinstance(
                result, list), "Test Posseg error on content: %s" % content
            print(
                " , ".join([w.word + " / " + w.flag for w in result]), file=sys.stderr)
    ...
    def testTokenize(self):
        for content in test_contents:
            ...
            assert isinstance(
                result, types.GeneratorType), "Test Tokenize Generator error"
            result = list(result)
            assert isinstance(
                result, list), "Test Tokenize error on content: %s" % content
            for tk in result:
                print("word %s\t\t start: %d \t\t end:%d" %
                      (tk[0], tk[1], tk[2]), file=sys.stderr)
